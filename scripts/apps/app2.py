...
# ── Reload scripts ───────────────────────────────────────────────────────────
# App2 (AI analytics) needs the full reload too:
#   01 schema  →  02 reference  →  03 external data  →  06 AI/pgvector
# The AI queries depend on netflow_features and syslog_embeddings which are
# created in 06_ai_analytics.sql, but that script needs the base data first.
...
ORDER BY anomaly_dimensions DESC, z_bytes DESC LIMIT 30"""
    },
    {
        ...
        "desc": "Behavioral clusters from MADlib kmeanspp — anomalous IPs land in small/outlier clusters",
        "sql": """SELECT
    a.cluster_id,
...
def api_reload_start():
    ...
    def _run():
        ...
        try:
            ...
            if _reload_running:
                _append_log("done", f"🎉 Full reload complete in {total}s — pgvector embeddings & MADlib features ready")
        finally:
            _reload_running = False
...
LIMIT 20;</textarea>
  ...
  <div class="pnl" id="pnl-4">
    <div class="sec-hdr">
      ...
      <h2>Data Reload</h2>
      <div class="d">Re-runs all 4 SQL scripts — refreshes timestamps so <code style="background:var(--bg);padding:1px 5px;border-radius:3px;font-family:'Courier New',monospace">now() - interval '6 hours'</code> queries return results</div>
    </div>
    ...
    <div class="reload-card">
      <div class="reload-hdr">
        <div>
          ...
          <div class="reload-sub">{{ workshop_dir }} — drops schema, reseeds, reloads ~95M rows, rebuilds pgvector embeddings &amp; MADlib features</div>
        </div>
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:12px;padding:18px 22px">
      ...
      <div style="font-size:13px;color:var(--muted);line-height:1.8">
        ...
        and rebuilds all derived tables — takes approximately <strong>5–8 minutes</strong>.
      </div>
...
function tbl(rows){
  if(!rows||!rows.length)
    return '<div class="empty">No results — data may need a reload (timestamps expired or 06_ai_analytics.sql not run)</div>';
